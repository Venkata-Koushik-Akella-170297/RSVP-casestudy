...
SELECT 
		...
		SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS null_published_date,
		SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS null_duration,
		SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS null_country,
		SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS null_world_wide_gross,
		SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS null_lang,
		SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS null_prod_comp
FROM movie;
...
+---------------+-------------------+ */
-- Type your code below:
SELECT year, COUNT(id) as mvis_per_year
FROM movie
GROUP BY year
ORDER BY year;

SELECT MONTH(date_published) AS month_num, COUNT(id) AS mvis_per_month 
FROM movie
...
-- getting a soft error while executing some queries. searched online and found that the following query will resolve the issue.
...
SELECT 
	genre, 
    ROUND(AVG(duration)) AS mvi_avg_durn
FROM genre AS g
